module Twig
  ...
  #
  # To enable profiling, pass the <tt>profile: true</tt> option to <tt>Twig::Template.parse</tt>. Then, after
  # <tt>Twig::Template#render</tt> is called, the template object makes available an instance of this
  # class via the <tt>Twig::Template#profiler</tt> method.
  #
  #   template = Twig::Template.parse(template_content, profile: true)
  ...
  #   end
  #
  # Profiler also exposes the total time of the template's render in <tt>Twig::Profiler#total_render_time</tt>.
  #
  ...
  class Profiler
    ...
    def start
      Thread.current[:twig_profiler] = self
      @render_start_at = Time.now
    end

    def stop
      Thread.current[:twig_profiler] = nil
